...
| `cacheMode` | string | 缓存策略：`"aware"`（默认，优化前缀以利用 KV Cache），`"strict"`（aware + 哈希验证），`"off"`（禁用）。仅限 DeepSeek | `"aware"` |
| `repositoryVisibility` | `"public"` \| `"private"` | 仓库可见性。`"public"` 会自动将 `/management/` 和 `/.agents/` 添加到 `.gitignore` | `"private"` |
| `githubToken` | string | 用于验证 GitHub Releases API 调用的令牌（可选 — 在频繁重启环境中避免 60 次/小时的速率限制） | *(无)* |
...
从 [Releases 页面](https://github.com/andrelncampos/dscode-public/releases) 下载二进制文件，解压后运行 `./dscode`。**需要 Node.js 24+。**
...
| **记录笔记** | 输入 `/notes-add` 创建带有标签、关联 spec 和截止日期的开发笔记。 |
...
| **开发笔记** | DsCode 内置的笔记系统。使用标签、spec 关联和截止日期记录技术债务、想法和任务。 | 在开发过程中使用 `/notes-add` 以避免丢失上下文。笔记存储在 `.dscode/notes.json` 中。 |
...
| **Prompt cache** | DeepSeek 缓存重复的上下文部分以减少 token 收费（KV Cache）。设置 `cacheMode` 进行优化。 | 自动发生。保持提示稳定以节省费用。退出时，DsCode 显示缓存效率（命中率和节省的美元）。 |
...
本项目源自 [DeepCode (lessweb/deepcode-cli)](https://github.com/lessweb/deepcode-cli)，原始许可为 MIT。原始版权声明保存在 [LICENSE](../LICENSE) 和 [NOTICE](../CHANGELOG.md) 中。

第三方依赖保持各自的许可证。请参阅 [NOTICE](../CHANGELOG.md) 了解依赖列表及其许可证。
