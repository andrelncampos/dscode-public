...
test("findExactSlashCommand returns null when nothing matches", () => {
  const items = buildSlashCommands();
  assert.equal(findExactCommandByPrefix(items, "/missing", "/"), null);
});

test("findExactSlashCommand returns built-in /new", () => {
  const items = buildSlashCommands();
  const item = findExactCommandByPrefix(items, "/new", "/");
  assert.ok(item);
...
test("findExactSlashCommand returns built-in /init", () => {
  const items = buildSlashCommands();
  const item = findExactCommandByPrefix(items, "/init", "/");
  assert.ok(item);
...
test("findExactSlashCommand returns built-in /continue", () => {
  const items = buildSlashCommands();
  const item = findExactCommandByPrefix(items, "/continue", "/");
  assert.ok(item);
...
test("findExactSlashCommand returns built-in /undo", () => {
  const items = buildSlashCommands();
  const item = findExactCommandByPrefix(items, "/undo", "/");
  assert.ok(item);
...
test("findExactSlashCommand returns built-in /skills", () => {
  const items = buildSlashCommands();
  const item = findExactCommandByPrefix(items, "/skills", "/");
  assert.ok(item);
...
test("findExactSlashCommand returns built-in /model", () => {
  const items = buildSlashCommands();
  const item = findExactCommandByPrefix(items, "/model", "/");
  assert.ok(item);
...
test("findExactSlashCommand returns built-in /raw", () => {
  const items = buildSlashCommands();
  const item = findExactCommandByPrefix(items, "/raw", "/");
  assert.ok(item);
...
test("findExactSlashCommand no longer matches skills (skills moved to #)", () => {
  const items = buildSlashCommands();
  assert.equal(findExactCommandByPrefix(items, "/code-review", "/"), null);
});
...
test("findExactHashCommand returns matching skill", () => {
  const items = buildHashCommands(skills);
  const item = findExactCommandByPrefix(items, "#code-review", "#");
  assert.ok(item);
